 ...
 */
export const robotChatGenerateWord = async (options: RobotChatOptions) => {
	const { sseControl, userContent, id, application_type, systemPrompt } = options;
	...
	// 根据内容类型获取合适的LLM模型
	const model = getLLM({}, hasImage, hasVideo);
	...
	for await (const chunk of stream) {
		...
		sseControl.sendData({
			status: 'process',
			stepName: '基本对话',
			data: { type: 'string', data: chunk },
		});
	}

	const fileName = Date.now().toString() + '.docx';
	
	await markdownToDocx(message, fileName);
